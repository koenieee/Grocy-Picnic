...
class PicnicSetupGrocy():
  ...
  picnic = PicnicAPI(username=config.picnic_user, password=config.picnic_passwd, country_code="NL", store=False)
  ...
  def setupQuantities(self):
      self.importIntoGrocy(config.picnic_quantity_list, "objects/quantity_units", "data")

  def setupUserfields(self):
      self.importIntoGrocy(config.picnic_userfields_list, "objects/userfields", "data")

  def setupStoreAndLocation(self):
      self.importIntoGrocy(config.picnic_stores_list, "objects/shopping_locations", "data")
      self.importIntoGrocy(config.picnic_location_list, "objects/locations", "data")
