...
class GrocyAPI():
    ...
    def getFromGrocy(self, apiPart: str):
        ...
        x = requests.get(url, headers=self.makeHeader(),
                         verify=self.verify_ssl)
        return json.loads(x.text)

    def putToGrocy(self, apiPart: str, postData: str):
        url = self.grocy_api_url + apiPart
        x = requests.put(url, data=postData,
                         headers=self.makeHeader(), verify=self.verify_ssl)
        if not x.ok:
            print("Failed put to grocy: " + x.response)
        else:
            return x

    def postToGrocy(self, apiPart: str, postData: str):
        url = self.grocy_api_url + apiPart
        x = requests.post(url, data=json.dumps(postData),
                          headers=self.makeHeader(), verify=self.verify_ssl)
        return x
    ...
    def addUserFieldToStore(self, shopping_locationId: str, userfield_data: str):
        return self.putToGrocy("userfields/shopping_locations/" + shopping_locationId, userfield_data)

    def changePictureFileNameProduct(self, product_id: str, filename: str):
        self.putToGrocy('objects/products/' + product_id,
    ...
    def addPictureToProduct(self, img_id: str):
        ...
        url = self.grocy_api_url + 'files/productpictures/' + \
        ...
        x = requests.put(
            url, data=html, headers=self.makeHeader(), verify=self.verify_ssl)
        if x.ok:
            return img_filename
        else:
