...
class PicNic():
    ...
    picnic = PicnicAPI(username=picnic_user,
                       password=picnic_passwd, country_code="NL", store=False)
    ...
    def __init__(self):
        ...
        self.getPicnicStoreData()  # get store ID and delivery ID with name Picnic
    ...
    def getPicnicStoreData(self):
        ...
        if len(found_store) > 0:
            ...
            self.picnic_last_delivery_id = found_store[0]["userfields"]["lastdeliveryid"]

    def setLastDeliveryID(self, delivery_id: str):
        self.grocy.addUserFieldToStore(
            str(self.picnic_store_id), "{\"lastdeliveryid\":\""+delivery_id+"\"}")

    def addPicnicIDToProduct(self, product_id: str, picnic_id: str):
        self.grocy.addUserFieldToProduct(
            str(product_id), "{\"picnic\":\""+picnic_id+"\"}")
    ...
    def addPicNicProductToGrocy(self, name: str, qu_id: str, picnic_id: str, imgId: str, price: str):
        ...
        if result.ok:
            ...
            print("Added picnic product to grocy: " +
                  grocy_product_id + ", picnic_id: " + picnic_id)
    ...
    def importLastPicnicDelivery(self):
        ...
        for picnic_item in self.picnic.get_deliveries()[0]["orders"][last_delivery_index]["items"][:self.picnic_numofelementsimport]:
            # picnic data:
            ...
            if grocy_id_existing_product != 0:
                ...
                print("Increasing stock: " + how_much +
                      " with id: " + sort_of_quantity)
                self.setInStock(grocy_id_existing_product,
                                how_much, price, order_data)
            else:
    ...
    def fillQuantity(self, quantity_text: str):
        ...
        if res[1] == "x":
            ...
            return [str(res[0]), str(self.matchPicnicQuantityByGrocyQuantity(res[1]))]

    def matchPicnicQuantityByGrocyQuantity(self, picnicQuantity: str):
        #print("picnicquanitity: " + picnicQuantity)
        found_quantity = ([self.quantities[key]
                           for key in self.quantities if picnicQuantity in key.lower()])
    ...
    def setInStock(self, grocy_id: str, stock: str, price: str, order_date):
        ...
        result = self.grocy.postToGrocy(
            'stock/products/'+grocy_id+'/add', post_data)
        if result.ok:
    ...
    def getAllGrocyProducts(self):
        ...
        for product in json_data:
            grocy_product = GrocyPicnicProduct(product["userfields"]["picnic"], product["id"], product["name"],
                                               product["picture_file_name"], product["tare_weight"], product["qu_id_stock"])
